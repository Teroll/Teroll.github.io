...
document.addEventListener("DOMContentLoaded", function (e) {
	var app = new _vue2.default({
		el: "#app",
		data: {
			currentTool: window.defaults.toolId,
			currentMaterial: window.defaults.materialId,
			diameter: window.defaults.diameter
		},
		...
		methods: {},
		watch: {
			diameter: function diameter(val, oldVal) {
				if (isNaN(val) || !val || val < 1) this.diameter = oldVal;
			}
		},
		computed: {
			inputData: function inputData() {
				return window.dataIn;
			},
			material: function material() {
				return this.inputData[this.currentTool].materials;
			},
			turnoverClean: function turnoverClean() {
				return Math.round(this.material[this.currentMaterial].turnoverClean * 1000 / (3.14 * this.diameter));
			},
			turnoverBlack: function turnoverBlack() {
				return Math.round(this.material[this.currentMaterial].turnoverBlack * 1000 / (3.14 * this.diameter));
			}
		}
	});
});
...
/* WEBPACK VAR INJECTION */}.call(__webpack_exports__, __webpack_require__(125), __webpack_require__(46), __webpack_require__(331).setImmediate))
